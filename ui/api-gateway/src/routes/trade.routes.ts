...
export function createTradeRoutes(redis: Redis) {
  return async (
    ...
    if (path === "/api/trade/limit" && method === "POST") {
      const { exchange, symbol, side, price, amount, reduceOnly, postOnly } =
        body as {
          exchange: string;
          symbol: string;
          side: "buy" | "sell";
      ...
      if (!exchange || !symbol || !side || !price || !amount) {
        json(res, 400, {
          error: "exchange, symbol, side, price, amount required",
        });
        return true;
      }
      ...
      const result = await callRest(redis, exchange, "createOrder", [
        symbol,
        "limit",
        side,
        amount,
    ...
    if (path === "/api/trade/order" && method === "DELETE") {
      const { exchange, symbol, orderId } = body as {
        exchange: string;
        symbol: string;
        orderId: string;
      };
      ...
      const result = await callRest(redis, exchange, "cancelOrder", [
        orderId,
        symbol,
      ]);
      if (result.success) json(res, 200, { ok: true, order: result.data });
    ...
    const openOrdersMatch = path.match(
    ...
    if (openOrdersMatch && method === "GET") {
      ...
      const result = await callRest(redis, exchange, "fetchOpenOrders", [
        symbol,
      ]);
    ...
    if (closeMatch && method === "POST") {
      await redis.xadd(
        ...
        "*",
        "data",
        JSON.stringify({ tradeId: closeMatch[1] }),
      );
    ...
    if (path === "/api/hedge/emergency" && method === "POST") {
      await redis.xadd(
        ...
        "*",
        "data",
        JSON.stringify({ trigger: "ui_button" }),
      );
    ...
    if (candlesMatch && method === "GET") {
      ...
      await redis.xadd(
        ...
        JSON.stringify({
          method: "fetchOHLCV",
          exchange,
          args: [symbol, tf, undefined, limit],
          replyTo,
        }),
      );
      ...
      while (Date.now() < deadline) {
        ...
        if (result.length > 0) {
          ...
          if (dataIdx !== -1 && fields[dataIdx + 1]) {
            const parsed = JSON.parse(fields[dataIdx + 1]!) as {
              success: boolean;
              data: unknown;
            };
      ...
      if (cached && q) {
        ...
        const filtered = markets.filter(
          (m) =>
            ...
            m.type === "swap" &&
            m.quote === "USDT" &&
            (m.base.includes(q) || m.symbol.includes(q)),
        );
    ...
    if (path === "/api/subscribe" && method === "POST") {
      ...
      // Notify risk engine to start monitoring
      await redis.xadd(
        "cmd:risk:subscribe",
        "MAXLEN",
        "~",
...
async function callRest(
  ...
  await redis.xadd(
    ...
    "*",
    "data",
    JSON.stringify({ method, exchange, args, replyTo }),
  );
